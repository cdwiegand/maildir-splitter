...
namespace maildir_splitter
{
    class State {
        ...
        public State(string[] args) {            
            ...
            if (bool.TryParse(GetEnvIfPresent("USE_FIND") ?? GetIfArgPresent(args, "-find"), out bool b)) useFind = b;
            if (bool.TryParse(GetEnvIfPresent("USE_FDUPES") ?? GetIfArgPresent(args, "-fdupes"), out bool b)) runFdupes = b;
        ...
        private static string GetEnvIfPresent(string envName) {
            if (Environment.Contains(envName)) return Environment.GetEnvironmentVariable(envName);
            return null;
        }
